  ...
  return (
    <svg viewBox="0 0 640 420" fill="none" aria-hidden="true" {...props}>
      ...
      <text x="84" y="352" fontSize="12" fill="#94a3b8" fontFamily="ui-sans-serif, system-ui">
        Deterministic enforcement decisions • Metadata-only audit events
      </text>
    </svg>
  );
}

function TestimonialCard({ quote, name, title, note }: { quote: string; name: string; title: string; note?: string }) {
  return (
...
export default function Home() {
  ...
  const deterministicTooltip = "Deterministic enforcement = same inputs + same policy + same context → same decision (with reason codes). Policy-evaluated and reproducible — not inferred from model behavior.";
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <h1 className="mt-6 text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-white">
                Deterministic governance for AI systems — <span className="text-teal-300">guardrails, budgets, evidence</span>
              </h1>
              ...
              <p className="mt-5 text-lg sm:text-xl leading-relaxed text-slate-300">
                TealTiger enforces controls at the point of execution. Protect model calls, contain spend, and emit audit events for
                security workflows — across providers.
              </p>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <a href="/capabilities" className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15 border border-slate-800 text-sm font-semibold">
                  Explore capabilities →
                </a>
                <a href="/enterprise" className="px-4 py-2 rounded-lg border border-slate-800 bg-slate-950 hover:bg-slate-900 text-sm font-semibold">
                  Enterprise evaluation
                </a>
                <a href="#get-started" className="px-4 py-2 rounded-lg bg-teal-600 hover:bg-teal-500 text-sm font-semibold text-white">
                  Get started
                </a>
              </div>
              ...
              <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="rounded-xl border border-slate-900 bg-slate-950/60 p-4">
                  <div className="text-sm font-semibold text-white">Deterministic guardrails</div>
                  <div className="mt-1 text-sm text-slate-300">PII, injection, safety checks</div>
                </div>
                <div className="rounded-xl border border-slate-900 bg-slate-950/60 p-4">
                  <div className="text-sm font-semibold text-white">Deterministic budgets</div>
                  <div className="mt-1 text-sm text-slate-300">limits across time windows</div>
                </div>
                <div className="rounded-xl border border-slate-900 bg-slate-950/60 p-4">
                  <div className="text-sm font-semibold text-white">Deterministic evidence</div>
            ...
            <div className="relative">
              ...
              <p className="mt-3 text-xs text-slate-500 text-center">Illustration: request path + deterministic enforcement boundary (conceptual)</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <section className="border-t border-slate-900 bg-slate-950">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14">
          ...
          <div className="mt-10 grid md:grid-cols-3 gap-6">
            <div className="rounded-2xl border border-slate-900 bg-slate-950/60 p-6 hover:bg-slate-950/80 transition-colors">
              <div className="flex items-center gap-3">
                <div className="rounded-xl border border-slate-900 bg-slate-950 p-2 text-teal-300"><IconWallet className="h-6 w-6" /></div>
                <h3 className="text-lg font-semibold text-white">Unbounded spend</h3>
              </div>
              <p className="mt-3 text-sm leading-relaxed text-slate-300">Deterministically enforce per-request limits and budgets to prevent runaway usage and surprise bills.</p>
            </div>

            <div className="rounded-2xl border border-slate-900 bg-slate-950/60 p-6 hover:bg-slate-950/80 transition-colors">
              <div className="flex items-center gap-3">
                <div className="rounded-xl border border-slate-900 bg-slate-950 p-2 text-teal-300"><IconShield className="h-6 w-6" /></div>
                <h3 className="text-lg font-semibold text-white">Data exposure</h3>
              </div>
              <p className="mt-3 text-sm leading-relaxed text-slate-300">Deterministically detect and handle sensitive input/output (PII) and reduce prompt injection risk.</p>
            </div>

            <div className="rounded-2xl border border-slate-900 bg-slate-950/60 p-6 hover:bg-slate-950/80 transition-colors">
              <div className="flex items-center gap-3">
                <div className="rounded-xl border border-slate-900 bg-slate-950 p-2 text-teal-300"><IconActivity className="h-6 w-6" /></div>
                <h3 className="text-lg font-semibold text-white">Control drift</h3>
              </div>
              <p className="mt-3 text-sm leading-relaxed text-slate-300">Prevent drift by enforcing the same policy decisions across services and environments, with structured audit events.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="testimonials" className="border-t border-slate-900 bg-slate-950">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="mt-12 grid md:grid-cols-3 gap-6">
            <TestimonialCard quote="The deterministic boundary-first approach is what we needed — decisions are explicit and the evidence is easy to route to our SOC." name="Security Engineering" title="Design partner" note="Anonymized" />
            <TestimonialCard quote="Budgets as a first-class deterministic control made the discussion with FinOps much simpler. It’s a practical guardrail, not a dashboard." name="Platform Engineering" title="Large enterprise" note="Anonymized" />
            <TestimonialCard quote="We liked that nothing is persisted by default. That single detail removes a lot of friction in deterministic security review." name="AppSec" title="Regulated environment" note="Anonymized" />
          </div>
          ...
          <div className="mt-8 rounded-2xl border border-slate-900 bg-slate-950/60 p-6">
            ...
            <p className="mt-2 text-sm text-slate-300 leading-relaxed">Use Enterprise, Compliance, Support, and Case studies pages for procurement and review conversations.</p>
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              <a href="/compliance" className="inline-flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-950 px-4 py-2 text-sm font-semibold text-slate-200 hover:bg-slate-900">Compliance →</a>
      ...
      <section id="eu-ai-act" className="border-t border-slate-900 bg-slate-950">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="mt-8 max-w-5xl mx-auto flex items-center justify-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-slate-800 bg-slate-900/60 px-4 py-2 text-sm text-slate-200">
              <span className="text-teal-300"><IconFlagEU className="h-5 w-5" /></span>
          ...
          <div className="mt-12 max-w-5xl mx-auto grid lg:grid-cols-3 gap-6">
            <div className="rounded-2xl border border-slate-900 bg-slate-950/60 p-6 lg:col-span-2">
              ...
              <a href="/compliance" className="mt-5 inline-flex text-sm font-semibold text-teal-300 hover:text-teal-200">See compliance enablement →</a>
            </div>
          ...
          <div className="mt-10 max-w-3xl mx-auto rounded-2xl border border-slate-900 bg-slate-950/60 p-6">
            <form onSubmit={onNewsletterSubmit} className="flex flex-col sm:flex-row gap-3">
              ...
              <input
                ...
                autoComplete="email"
              />
              <button type="submit" className="rounded-lg bg-teal-600 hover:bg-teal-500 px-4 py-3 text-sm font-semibold text-white">Subscribe</button>
            </form>
            ...
            {status === 'invalid' ? (
              ...
              <p className="mt-3 text-sm text-slate-400">No backend required right now — subscription uses your email client (mailto). We will never ask for API keys or sensitive data.</p>
            )}

            <div className="mt-5 flex flex-wrap gap-3">
              <a href="https://blogs.tealtiger.ai" className="text-sm font-semibold text-teal-300 hover:text-teal-200" rel="noreferrer">Read the blog →</a>
              <a href="/support" className="text-sm font-semibold text-teal-300 hover:text-teal-200">Support →</a>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="mt-10 grid lg:grid-cols-2 gap-6 max-w-5xl mx-auto">
            <div className="rounded-2xl border border-slate-900 bg-slate-950/60 p-6">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <a href="/deployments" className="text-sm text-teal-300 hover:text-teal-200">Deployments</a>
                <a href="/integrations" className="text-sm text-teal-300 hover:text-teal-200">Integrations</a>
                <a href="/enterprise" className="text-sm text-teal-300 hover:text-teal-200">Enterprise</a>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14">
          <div className="grid md:grid-cols-4 gap-10 mb-10">
            <div>
              ...
              <p className="text-sm text-slate-300 leading-relaxed">Deterministic runtime controls for AI applications — guardrails, budgets, and audit events.</p>
            </div>

            <div>
              <h4 className="text-sm font-semibold text-white mb-3">Enterprise</h4>
              <ul className="space-y-2 text-sm">
                <li><a href="/enterprise" className="text-slate-300 hover:text-white transition-colors">Enterprise overview</a></li>
                <li><a href="/compliance" className="text-slate-300 hover:text-white transition-colors">Compliance enablement</a></li>
                <li><a href="/support" className="text-slate-300 hover:text-white transition-colors">Support</a></li>
                <li><a href="/roadmap" className="text-slate-300 hover:text-white transition-colors">Roadmap</a></li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                <li><a href="/capabilities" className="text-slate-300 hover:text-white transition-colors">Capabilities</a></li>
                <li><a href="/deployments" className="text-slate-300 hover:text-white transition-colors">Deployments</a></li>
                <li><a href="/integrations" className="text-slate-300 hover:text-white transition-colors">Integrations</a></li>
                <li><a href="/trust" className="text-slate-300 hover:text-white transition-colors">Trust</a></li>
              </ul>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                ...
                <li><a href="https://docs.tealtiger.ai" className="text-slate-300 hover:text-white transition-colors" rel="noreferrer">Docs</a></li>
                <li><a href="https://blogs.tealtiger.ai" className="text-slate-300 hover:text-white transition-colors" rel="noreferrer">Blog</a></li>
              </ul>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between border-t border-slate-900 pt-8">
            ...
            <div className="text-xs text-slate-400">Contact: <span className="text-slate-300">reachout@tealtiger.ai</span></div>
          </div>
        </div>
      </footer>
    </div>
